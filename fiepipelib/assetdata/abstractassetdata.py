import hashlib
import sqlite3
import fiepipelib.assetdata.assetdatabasemanager
...
class abstractmultimanager(object):
    ...
    def __init__(self, workingAsset:fiepipelib.gitstorage.workingasset.workingasset, dataManagers = []):
        self._dataManagers = dataManagers.copy()
        for dm in dataManagers:
            assert isinstance(dm, abstractdatamanager)
    ...
    def _GetHashOfDump(self):
        fname = self._GetDBDumpFilename()
        path = pathlib.Path(fname)
        dumphash = fiepipelib.assetdata.assetdatabasemanager.md5file(str(path.absolute()))
        return dumphash
    
    def _WriteHashToDB(self, hash, conn:sqlite3.Connection):
        h = fiepipelib.assetdata.assetdatabasemanager.FromParams(self._GetDBFilename(),hash)
        man = fiepipelib.assetdata.assetdatabasemanager.AssetDatabaseManager(self._workingAsset)
        man.Set([h],conn)
    ...
    def _UpToDateAttach(self, connection:sqlite3.Connection = None):
        ...
        #they both exist
        #before calculating a hash, we check for an existing entry. 
        
        dbhashmanager = fiepipelib.assetdata.assetdatabasemanager.AssetDatabaseManager(self._workingAsset)
        ...
        dbhash = dbhashes[0]
        assert isinstance(dbhash, fiepipelib.assetdata.assetdatabasemanager.databasehash)

        #finally, we md5 and check equality.
        hashofdump = self._GetHashOfDump()
    ...
    def AttachToConnection(self, connection:fiepipelib.assetdata.assetdatabasemanager.Connection):
    ...
    def _CreateTables(self, cur):
        for m in self._dataManagers:
            assert isinstance(m, abstractdatamanager)
            m._CreateTable(cur)
...
class abstractdatamanager(object):
    ...
    def GetMultiManager(self):
        assert isinstance(self._multiManager, abstractmultimanager)
    ...
    def GetAll(self, connection:fiepipelib.assetdata.assetdatabasemanager.Connection):
        return self._Get(colNamesAndValues=[],conn=connection.GetDBConnection())

    def Set(self, items, connection:fiepipelib.assetdata.assetdatabasemanager.Connection):
...
class AbstractFileVersionManager(abstractdatamanager):
...
class AbstractFileVersion(object):
    ...
    def GetWorkingAsset(self) -> fiepipelib.gitstorage.workingasset.workingasset:
        return self._workingAsset
...
def AbstractFileVersionFromJSON(afv:AbstractFileVersion, data:typing.Dict, workingAsset:fiepipelib.gitstorage.workingasset.workingasset):
    afv._version = data['version']
    afv._workingAsset = workingAsset
    return afv

def AbstractFileVersionFromParameters(afv:AbstractFileVersion, version:str, workingAsset:fiepipelib.gitstorage.workingasset.workingasset):
