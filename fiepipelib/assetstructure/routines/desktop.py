...
class AbstractDesktopProjectAssetBasePath(AbstractAssetBasePath[TABP], typing.Generic[TABP], abc.ABC):
    ...
    async def pre_children_automanage_routine(self, feedback_ui: AbstractFeedbackUI, entity_config: LegalEntityConfig,
        ...
        if entity_config.get_mode() == LegalEntityMode.USER_WORKSTATION:
            ...
            if self_is_dirty:
                return AutoManageResults.DIRTY
            else:
                return AutoManageResults.CLEAN
        else:
            await feedback_ui.error("Unsupported entity auto-manager mode.")
    ...
    async def post_children_automanage_routine(self, feedback_ui: AbstractFeedbackUI, entity_config: LegalEntityConfig,
                                 container_config: ContainerAutomanagerConfigurationComponent, children_results:AutoManageResults) -> AutoManageResults:
        """
        ...
        if entity_config.get_mode() == LegalEntityMode.USER_WORKSTATION:
            ...
            self_is_dirty = self.is_dirty(True,True,True,False)
            children_dirty = (children_results == AutoManageResults.DIRTY) or (children_results == AutoManageResults.DIRTY_AND_UNPUBLISHED_COMMITS)
            is_working_on = self_is_dirty or children_dirty
            children_unpublished_commits = (children_results == AutoManageResults.UNPUBLISHED_COMMITS) or (children_results == AutoManageResults.DIRTY_AND_UNPUBLISHED_COMMITS)
            ...
            if is_detached:
                ...
                if self_is_dirty:
                    ...
                    await feedback_ui.warn("You are in danger of divergence because you are working on a detached head.")
                    ...
                    if children_unpublished_commits:
                        ...
                        await feedback_ui.error("Cannot safely resolve automatically because we might lose childrens' commits.  User intervention is required.")
                        return AutoManageResults.CANNOT_COMPLETE
                    elif children_dirty:
                        await feedback_ui.error("Children are dirty.")
                        await feedback_ui.error("Cannot safely resolve automatically because we might lose childrens' changes in progress.  User invervention is required.")
                        return AutoManageResults.CANNOT_COMPLETE
                    else:
            ...
            if remote_is_ahead:
                ...
                if is_working_on:
                    await feedback_ui.warn("This asset is being worked on.  User may want to merge in changes manually.")
                    if children_unpublished_commits:
                        await feedback_ui.warn("Its children have unpublished commits and that should be resolved before merging/pulling this asset.")
                    if children_dirty:
                        await feedback_ui.warn("Its children are dirty and should be resolved before merging/pulling this asset.")
                    ...
                    if not pull_success:
                        ...
                        if remote_is_behind:
                            ...
                            if push_success:
                                ...
                                await feedback_ui.output("Merge succeeded but push failed.  Continuing auto-management.")
                                ret = get_worse_enum(ret, AutoManageResults.UNPUBLISHED_COMMITS)
                        else:
