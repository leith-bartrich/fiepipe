...
class asset(object):
    ...
    This object only represents the asset's existence across storge volumes.

    It is constructed only from an id.  Which may be the only thing you know about an asset at some points in time.

    An ID might be obtained from:

    -Backing Storage: workingasset.GetAsset() which in turn comes from a git submodule, which in turn comes from a git Repo which in turn comes from a localworkingdirectoryroot.
    -Archival Storage.
