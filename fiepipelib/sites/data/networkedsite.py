...
def FromJSONData(data):
    ret = networkedsite()
    fiepipelib.sites.data.abstractsite.FromJSONData(ret, data)
...
def ToJSONData(site):
    ...
    ret = {}
    fiepipelib.sites.data.abstractsite.ToJSONData(site, ret)
...
def FromParameters(name, fqdn, stateServer, port, publicKey):
    ret = networkedsite()
    fiepipelib.sites.data.abstractsite.FromParameters(ret, name, fqdn)
    ret._stateServer = stateServer
    ret._port = port
    return ret


class localregistry(fiepipelib.sites.data.abstractsite.abstractlocalregistry):
...
class networkedsite(fiepipelib.sites.data.abstractsite.abstractsite):
